...
\begin{enumerate}[1.]
	...
	\begin{enumerate}[(a)]
		...
		\item Find bases $P_A,Q_A$ and $P_B,Q_B$ s.t. $\langle P_A,Q_A\rangle$ = $E[2^{e_A}]$ and $\langle P_B,Q_B\rangle$ = $E[3^{e_B}]$ %wie schwierig ist das?
	\end{enumerate}
...
How does the SIKE specification for NIST PQCC differ from SIDH?

\begin{itemize}[\textbullet]
	\item Instead of a Weierstrass curve a Montgovery curve $E_0(\mathbb{F}_{p^2}): y^2 = x^3 + 6x^2 +x$ is being used by default
\end{itemize}
...
\begin{frame}{Composition chains of isogenies}
	...
	% welche Verbesserungen gab es in Runde 2? Wie schnell ist SIKE geworden?
\end{frame}
...
%Nachteil: sehr kompliziert, Parameter müssen mit Bedacht ausgewählt werden (SIDH spec 1.3.2.)
% SIKE spec chapter 6
\end{frame}

\begin{frame}{Open questions}
\begin{itemize}
	\item Why do cryptosystem designers always choose extremely complicated problems that are not even NP-complete?
