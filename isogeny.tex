...
\textbf{The Good:}
\begin{enumerate}[(i)]
	\item Very small key sizes
	\item No commutative group action
	\item Potential for performance improvements
\end{enumerate}
\vspace{5mm}
\textbf{The Bad:}
\begin{enumerate}[(i)]
	\item Comparably poor, yet practical performance
	\item Very recent
\end{enumerate}
...
\begin{itemize}[\textbullet]
	\item Find the line intersecting E at P and Q
	\item If $P=Q$, find tangent of E with the same gradient like E at point P
	\item Plug line equation into E and solve to find the  point $R$ of intersection
	\item Flip y-coordinate to get $R^{\prime}$
\end{itemize}
...
\begin{frame}{Elliptic Curves over finite fields}
	\begin{itemize}[\textbullet]
		...
		\item The set of points of E with coordinates in $\mathbb{F}_p$ (for a prime $p>3$) is the set $E(\mathbb{F}_p)=\{(x,y):x,y\in \mathbb{F}_p$ satisfy $y^2=x^3+Ax+B \} \cup \{\mathcal{O}\}$
		...
		\item It follows for all supersingular curves: $\#E(\mathbb{F}_{q^n})\equiv 1 \bmod p$ for all $n\in \mathbb{N}$ and $p>3$ % makes it easy to split E into 2 torsion groups
	\end{itemize}
...
\begin{itemize}[\textbullet]
	...
	\item Their endomorphism ring is not commutative which  prevents quantum attacks
\end{itemize}
\end{frame}
...
\begin{itemize}[\textbullet]
	...
	\item Endomorphism $E[m] = \{P\in E: mP = \mathcal{O}\}$ is also called the m-torsion group of E and includes all points of order $m$ in $E$
...
\begin{enumerate}[1.]
	...
	\begin{enumerate}[(a)]
		\item Choose $2^{e_A},3^{e_B}$, search for a prime $p=2^{e_A}3^{e_B}f- 1$ %and compute $q=p^2$
...
\begin{itemize}[\textbullet]
	...
	\item All known quantum and classical attacks are exponential
	\item Security research on isogeny-based cryptosystems might not be as extensive as for less complicated cryptosystems
	\item Great candidate for hybrid cryptosystems
	\item Current performance worse than alternatives, but potential for improvements
\end{itemize}
\vfill
Open Questions
\begin{itemize}[\textbullet]
	\item It seems hard to qualify the reliability of SIKE's security assumptions for non-experts in the field
