...
\begin{enumerate}[1.]
	...
	\end{enumerate}
	\item Commutative Action:
	\begin{enumerate}[(a)]
			\item Alice computes the image $\{\phi_A(P_B),\phi_A(Q_B)\} \subset E/A$ and sends it to Bob, Bob sends Alice $\{\phi_B(P_A),\phi_B(Q_A)\} \subset E/B$
			\item Now Alice and Bob both are able to compute the isogeny $E/AB$ with kernel $\langle [m_A]P_A+[n_A]Q_A,[m_B]P_B+[n_B]Q_B \rangle$. Their secret key is the j-invariant $j(E/AB)$
	\end{enumerate}
